...
public abstract class BlockIronFurnaceContainerBase extends AbstractContainerMenu {
    ...
    public int getSideMode(int side) {
        return this.te.furnaceSettings.get(side);
    }
        ...
        if (slot != null && slot.hasItem()) {
                ...
                if (index != 6 && index != 3 && index != 4 && index != 5)
                {
                    if (te.getItem(3).getItem() instanceof ItemAugmentSmoking)
                    {
                        if (te.getSmokingBurn(itemstack1) > 0) {
                            if (!BlockIronFurnaceTileBase.getCraftingRemainingItem(itemstack1).isEmpty())
                            {
                    ...
                    {
                        if (BlockIronFurnaceTileBase.isItemFuel(itemstack1, RecipeType.SMELTING, this.te.getLevel()) && !(itemstack1.getItem() instanceof ItemHeater)) {
                            if (!this.moveItemStackTo(itemstack1, 6, 7, false)) {
                                return ItemStack.EMPTY;
                            }
                        }
            ...
            if (te.isFurnace())
            {
                ...
                if (index == 2) {
                    ...
                    } else if (BlockIronFurnaceTileBase.isItemFuel(itemstack1, RecipeType.SMELTING, this.te.getLevel())) {
