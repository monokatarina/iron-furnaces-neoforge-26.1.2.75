...
public class IronFurnacesJEIPlugin implements IModPlugin {
	...
	public void registerRecipes(IRecipeRegistration registration) {
			...
			{
				int burntime = BlockIronFurnaceTileBase.getBurnTime(new ItemStack(item), RecipeType.SMELTING);
				if (burntime > 0)
				{
			...
			if (minecraft.level != null) {
				List<RecipeHolder<GeneratorRecipe>> list = minecraft.level.getRecipeManager().getAllRecipesFor(Registration.GENERATOR_RECIPE_TYPE.get()).stream().toList();
